...
public class TableDescriptor {
    ...
    public static class ColumnDescriptor {
        ...
        public ColumnDescriptor(Field field, PropertyDescriptor pd, String columnName, int length, int type,
                boolean pk, boolean autoIncrease) {
            super();
            this.field = field;
            this.pd = pd;
    ...
    public TableDescriptor(Class clazz) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            columns.add(new ColumnDescriptor(f, pd, columnName, columnLength, type, c.pk(), c.auto_increase()));
        }
        ...
        if (pkColumns.size() != 1)
            throw new IllegalStateException("BaseSpringDao only support solitary pk column");
    }
    ...
    private int detectType(Field f) {
        ...
        throw new IllegalStateException("Failed to detect field db type. Field: " + f.toGenericString());
    }
